...
jQuery(document).ready(function(){
  ...
  $.extend(SurveyorAjax, {
    ...
    prototype: {
      ...
      showOrHideQuestions: function( responseText ) {
        ...
        jQuery.each(responseText.hide, function(){
          ...
          if ( response_element.length > 0 ) {
            ...
            if ( collapsable_buttons.length ){
              collapsable_buttons.addClass('collapsed');
            }

            if ( detail_info.length ){
      ...
      showElement: function( id ) {
         var parentGroup = this.getParentGroup(id);
         this.show(id, this.hiddenClassName(id));
         if (parentGroup.length && parentGroup.hasClass('g_hidden')) {
           this.showGroup(parentGroup);
         }
      },
      ...
      hideElement: function( id ) {
        ...
        if (parentGroup.length && $('#' + id).siblings('.q_grid').filter(':not(.q_hidden)').length == 0) {
           this.hideGroup(parentGroup);
        }
        this.removeUselessResponse(id);
      },

      getParentGroup: function ( id ) {
        return $('#' + id).closest('fieldset[id^="g_"]');
      },
 
      hideGroup: function( groupElement ) {
         groupElement.addClass('g_hidden');
      },
 
      showGroup: function( groupElement ) {
        groupElement.removeClass('g_hidden');
      },
  ...
  jQuery(document).on("change", "form#survey_form input, form#survey_form select, form#survey_form textarea", function(){
    ...
    console.log(this);
    console.log($surveyorAjax.sendData($(this)));
  });
